...
def plot_dict_palette(output_dict, output_file, root_folder):
    ...
    epochs = [key for key, _ in output_dict.items()][:-1]

    # OJECTIVE FUNCTION HANDLED INDEPENDENTLY    
    if ("train_mse_loss" in output_dict[1]):
        objective_function = "MSE"
        obj_loss = np.array([output_dict[epoch]["train_mse_loss"] for epoch in epochs])
    else:
        objective_function = "MAE"
        obj_loss = np.array([output_dict[epoch]["train_mae_loss"] for epoch in epochs])

    mae = np.array([output_dict[epoch]["val_mae"] for epoch in epochs])
    mse = np.array([output_dict[epoch]["val_mse"] for epoch in epochs])
    ssim = np.array([output_dict[epoch]["val_SSIM"] for epoch in epochs])
    psnr = np.array([output_dict[epoch]["val_PSNR"] for epoch in epochs])
    val_step_mse = np.array([output_dict[epoch]["val_step_mse"] for epoch in epochs])
