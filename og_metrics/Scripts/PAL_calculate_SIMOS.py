...
def compare_lists(listA, listB):
    listA = sorted([elm.split("/")[-1].split("_")[1] for elm in listA])
    listB = sorted([elm.split("/")[-1].split("_")[1] for elm in listB])
...
def calculate_SIMOS(listA, listB):
    ...
    for (currA, nextA), (currB, nextB) in zip(zip(listA[:-1],listA[1:]),zip(listB[:-1],listB[1:])):
        ...
        try:
            ...
            curr_slice = int(currA.split("/")[-1].split("-")[1].split(".")[0])
            next_slice = int(nextA.split("/")[-1].split("-")[1].split(".")[0])
