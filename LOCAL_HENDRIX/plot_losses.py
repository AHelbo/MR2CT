...
def plot_log_p2p(file_path, root_folder):
    ...
    for value in epochs:
        ...
        means.append([value, mean_G_GAN, mean_G_L1, mean_D_real, mean_D_fake])
    means = np.array(means)
...
def harry_plotter(data_dict, root_folder, title):
    ...
    for i, (val, vals) in enumerate(data_dict.items()):
        x_values = vals[:,0]
        y_values = vals[:,1]
        axs[0,i].plot(x_values, y_values, marker='o', linestyle='-', markersize = 2)
        axs[0,i].set_title(f"{val}")
        axs[0,i].set_xlabel("Epoch")
        axs[0,i].set_ylabel(f"{val}")

        axs[1,i].plot(x_values, smoothed_data[val])
        axs[1,i].set_xlabel("Epoch")
        axs[1,i].set_ylabel(f"Rolling average ({val})")        
