...
def write_to_file(output_file_path, epoch, SEC):
    with open(output_file_path, "a") as file:
        file.write(f"EPOCH {epoch} SDC {SEC}\n")

if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python3 calculate_SDC.py <pathA> <pathB> <epoch> <path to output file>")

    else:
        print("Calculating SDC")
        ...
        SDC = calculate_SDC(real_list, fake_list)

        # write the result as SDC (slice-wise difference comparison) to file
        epoch = sys.argv[3]
        path_to_output_file = sys.argv[4]
        write_to_file(path_to_output_file, epoch, SDC)
