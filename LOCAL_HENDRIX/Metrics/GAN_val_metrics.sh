...
if [ "$#" -lt 1 ] || [ "$#" -gt 6 ]; then
    echo "Usage: $0 [model name] [start epoch <5,..,5*n>, default=5] [end epoch <5,..,5*n>, default=5] [use gpu <0,1>, default=0] [n_threads <1,..,inf>, default=1]"
    exit 1
fi
...
process_epoch(){
    ...
    results_folder="$parent_dir/GAN/results/$model_name/test_$epoch/"
    if [[ -d "$results_folder" ]] && (( $(find "$results_folder/images/" -type f | wc -l) > 27000 )); then
        echo "Folder '$results_folder' exists and contains more than 27000 files."
    else
        echo "Folder '$results_folder' does not exist or contains 27000 or fewer files."
        # kald test.py på hver af disse tal og på hver mode, for at generere fake-Bs
        cd "$parent_dir/GAN"
        python3 "$parent_dir/GAN/test.py" --dataroot ./datasets/val_nc$input_nc --name $model_name --model pix2pix --input_nc $input_nc --output_nc 1 --num_test -1 --epoch $epoch --gpu_ids $gpu_id
    fi
    ...
    for real_file in "$parent_dir/GAN/results/$model_name/test_$epoch/images/"*_real_B.png; do
